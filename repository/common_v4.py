...
from json_to_txt import process_and_write_json
...
def process_foreign_word(index_data,foreign_words_data,words_info,verbs_data):
    ...
    for i,foreign_word in enumerate(foreign_words_data):
        ...
        main_verb,flag=process_construction_cp(relation_head,verbs_data,flag,main_verb)
        # for verb in verbs_data:
        #     # if len(verb[4]) > 0 and verb[4].strip().split(':')[1] == 'main' or verb[4].strip().split(':')[1] == 'rcelab' or verb[4].strip().split(':')[1] == 'rcdelim':
    ...
    for noun in nouns:
        ...
        if '+' in noun[1]:
            ...
            if 'era' in seman_data and noun[2] in ('dom','moy','yoc'):
                ...
                processed_postpositions_dict[index] = postposition
                case='d'
            elif 'moy' in seman_data and noun[2] == 'dom':
                postposition=None
                processed_postpositions_dict[index] = postposition
                case='d'
...
def all_processed_cat(index_data,gnp_data,seman_data,depend_data,spkview_data,sentence_type,words_info,k1_not_need,has_changes):
...
def identify_constructions(root_words,index_data,depend_data1,spkview_data,construction_data):
    for i, concept in enumerate(root_words):
        if 'conj' in concept :
            flag_conj=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row(i,construction_data,depend_data1,index_data)
            # del(gnp_data[i])
            ...
            flag_span=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row_span(i,construction_data,depend_data1,index_data)
            # del(gnp_data[i])
        elif 'cp' in concept:
            flag_cp=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row_cp(i,construction_data,depend_data1,index_data,spkview_data)
        elif 'meas' in concept:
            flag_meas=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row_meas(i,construction_data,depend_data1,index_data)
            # del(gnp_data[i])
            ...
            flag_waw=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row_waw(i,construction_data,depend_data1,index_data)
            ...
            flag_cal=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1=construction_row_calender(i,construction_data,depend_data1,index_data)
        elif 'spatial' in concept:
            flag_spatial=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1= construction_row_spatial(i,construction_data,depend_data1,index_data)
            ...
            flag_temporal=True
            HAS_CONSTRUCTION_DATA = True
            depend_data1= construction_row_temporal(i,construction_data,depend_data1,index_data)
            flag_span=True
    return depend_data1,flag_conj,flag_disjunct,flag_span,flag_cp,flag_meas,flag_rate,flag_waw,flag_cal,flag_spatial,flag_xvanxva,flag_temporal
...
def construction_row_temporal(i,construction_data1,depend_data,index_data1):
    dep1=depend_data[i]
    for j, text in enumerate(construction_data1):
...
def construction_row_cp(i,construction_data1,depend_data,index_data1,spkview_data):
    ...
    for j, text in enumerate(construction_data1):
        if 'kriyAmUla' in text and dep1:
            # op_indexes.append(i)
            depend_data[j]=dep1
        elif 'verbalizer' in text and dep1:
            depend_data[j]=dep1
...
def process_constructions(processed_words, root_words, construction_data, depend_data, gnp_data, index_data, 
                          conj_concept, flag_conj, flag_disjunct, flag_span, flag_rate, flag_spatial, flag_xvanxva):
    """
    Process various constructions based on flags.

    Args:
        processed_words (list): List of processed words.
        root_words (list): List of root words.
        construction_data (list): Construction data.
        depend_data (list): Dependency data.
        gnp_data (list): GNP data.
        index_data (list): Index data.
        conj_concept (object): Conjunction concept.
        flag_conj (bool): Flag for conjunction.
        flag_disjunct (bool): Flag for disjunction.
        flag_span (bool): Flag for span.
        flag_rate (bool): Flag for rate.
        flag_spatial (bool): Flag for spatial.
        flag_xvanxva (bool): Flag for XvanXva.

    Returns:
        tuple: Updated processed words and flags.
    """
    if flag_conj or flag_disjunct:
        processed_words, flag_conj, flag_disjunct = process_construction_conj_disjunct(
            ...
            gnp_data, index_data, conj_concept, flag_conj, flag_disjunct
        )
    if flag_span:
    ...
    return (processed_words, flag_conj, flag_disjunct, flag_span, 
            flag_rate, flag_spatial, flag_xvanxva)
